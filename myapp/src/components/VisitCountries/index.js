...
import VisitCountriesItem from '../VisitCountriesItem/index'
...
class VisitCountries extends Component {
  ...
  clickCountry = id => {
    ...
    const updatedCountriesList = countriesList.map(each => {
      if (each.id === id) {
  ...
  render() {
    const {countriesList} = this.state
    const visitedCountriesList = countriesList.filter(each => each.isVisited)
    return (
